...
import { EXTRA_DELETE_SENTENCES, EXTRA_SENTENCES, EXTRA_SYMBOLS, EXTRA_WORDS } from "./utils";
...
const relativeTest: Test = {
	...
	updateBuffer: () => {
		...
		relativeTest.targetPosition = Math.floor(Math.random() * relativeTest.textBuffer.length);
		const randomTarget =
...
let mixedTest: Test = {
	...
	updateBuffer: () => {
		const testTypes = [TestType.WORDS, TestType.CONTAINERS, TestType.RELATIVE, TestType.MOVEMENT];
